...
# WRITE YOUR SOLUTION BELOW:
NUM_INPUTS=$(bitcoin-cli -regtest -rpcwallet=btrustwallet decoderawtransaction $BASE_TX | jq -r ".vin | length")
check_cmd "Input counting" "NUM_INPUTS" "$NUM_INPUTS"

NUM_OUTPUTS=$(bitcoin-cli -regtest -rpcwallet=btrustwallet decoderawtransaction $BASE_TX | jq -r ".vout | length")
...
SIMPLE_RAW_TX=$(bitcoin-cli -named -regtest -rpcwallet=btrustwallet  createrawtransaction inputs="$SIMPLE_TX_INPUTS" outputs="$SIMPLE_TX_OUTPUTS")
...
PARENT_TXID=$(bitcoin-cli  -regtest -rpcwallet=btrustwallet decoderawtransaction $RAW_TX | jq -r ".txid")
...
CHILD_RAW_TX=$(bitcoin-cli -named -regtest -rpcwallet=btrustwallet  createrawtransaction inputs="$CHILD_INPUTS" outputs="$CHILD_OUTPUTS")
...
SECONDARY_TXID=$(bitcoin-cli  -regtest -rpcwallet=btrustwallet decoderawtransaction $SECONDARY_TX | jq -r ".txid")
...
output_value=$(bitcoin-cli -regtest -rpcwallet=btrustwallet decoderawtransaction $SECONDARY_TX | jq -r ".vout[0].value")
...
TIMELOCK_TX=$(bitcoin-cli -regtest -rpcwallet=btrustwallet createrawtransaction "$TIMELOCK_INPUTS" "$TIMELOCK_OUTPUTS")
